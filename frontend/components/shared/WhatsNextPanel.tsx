...
export default function WhatsNextPanel({ role, alerts, studentStatus }: WhatsNextPanelProps) {
  ...
  const renderStudentView = () => {
    if (studentStatus === 'offered') {
      return (
        <div className="bg-[#EDE9FE] border border-[#DDD6FE] rounded-lg p-5">
    ...
    if (studentStatus === 'inProgress') {
      return (
        <div className="bg-[#DCFCE7] border border-[#BBF7D0] rounded-lg p-5">
          <div className="flex gap-3">
    ...
    return (
      <div className="bg-white border border-[#E2E8F0] rounded-lg p-5">
        <h4 className="text-sm font-bold text-[#0F172A] mb-1">What's Next?</h4>
        <p className="text-xs text-[#475569]">
          Browse available verified internship postings and submit your application. Be sure to check details and eligibility criteria.
        </p>
        <Link
          href="/student"
          className="inline-flex items-center gap-1.5 mt-3 text-xs font-bold text-[#5B21B6] hover:underline"
  ...
  const renderFacultyView = () => {
    return (
      <div className="bg-white border border-[#E2E8F0] rounded-lg p-5">
        <h4 className="text-sm font-bold text-[#0F172A] mb-1">Mentor Actions</h4>
        <p className="text-xs text-[#475569]">
          Check your assignment queue to accept/reject new students. Review active student profiles to verify weekly evidence submissions and review risk flags.
        </p>
        <Link
          href="/faculty"
          className="inline-flex items-center gap-1.5 mt-3 text-xs font-bold text-[#5B21B6] hover:underline"
        >
          View Mentor Queue <ArrowRight className="w-3.5 h-3.5" />
        </Link>
      </div>
    );
  };
  ...
  const renderCompanyView = () => {
    return (
      <div className="bg-white border border-[#E2E8F0] rounded-lg p-5">
        <h4 className="text-sm font-bold text-[#0F172A] mb-1">Company Dashboard Panel</h4>
        <p className="text-xs text-[#475569]">
          Create structured criteria postings. Shortlist applications, extend offers, and evaluate final PPO recommendations once internships are complete.
        </p>
        <Link
          href="/company/postings/new"
          className="inline-flex items-center gap-1.5 mt-3 text-xs font-bold text-[#EA580C] hover:underline"
        >
          Post New Internship <ArrowRight className="w-3.5 h-3.5" />
        </Link>
      </div>
    );
  };
  ...
  const renderHodView = () => {
    return (
      <div className="bg-white border border-[#E2E8F0] rounded-lg p-5">
        <h4 className="text-sm font-bold text-[#0F172A] mb-1">Department Oversight</h4>
        <p className="text-xs text-[#475569]">
          Read-only access to department-level placement statistics, skills gap reports, and active student progress indicators.
        </p>
        <Link
          href="/hod"
          className="inline-flex items-center gap-1.5 mt-3 text-xs font-bold text-[#5B21B6] hover:underline"
        >
          Monitor Department <ArrowRight className="w-3.5 h-3.5" />
        </Link>
      </div>
    );
  };
